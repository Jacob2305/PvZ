...
public class Repeater extends Plant
{
    ...
    public Repeater(int xCoor, int yCoor)
    {
        super(xCoor, yCoor);
        otherCurrTime = System.currentTimeMillis() + 100;
        this.setCost(200);
        img1 = new ImageIcon(Lawn.class.getResource("RepeaterCos.png"));
    }
    ...
    public void act(ArrayList<Zombie> z, ArrayList<Projectile> p, ArrayList<Plant> pl, Lawn l, ArrayList<Sun> s, ArrayList<Explosions> e)
    {
        if(this.getX() > 0 && this.getY() > 0)
        {
            ...
            //this part is a paste of the attack method with different system.currentmillis()
            //so it fires again 
            for(int i = 0; i < z.size(); i++)
            {
                ...
                if(this.getY() > zY && this.getY() < zY + zH)
                {
                    ...
                    {
                        if(System.currentTimeMillis() >= otherCurrTime + this.getFireRate())
                        {
                            attack(p);
                            otherCurrTime = System.currentTimeMillis() + 100;
                            return;
    ...
    public void drawSelf(Graphics g)
    {
        ...
        g2d.drawImage(img, this.getX(), this.getY() - 15, this.getW(), this.getH() + 30, null);
    }    
}